...
`);
  ...
  test("parses output keys without defaults", () => {
    ...
    expect(parseOutputKeys(" AWS_REGION, VITE_SITE_URL ,, AWS_REGION ")).toEqual([
      "AWS_REGION",
      "VITE_SITE_URL",
    ]);
  });
...
describe("GitHub Actions export", () => {
  ...
  test("can skip GitHub-reserved env keys while exporting app keys", async () => {
    const dir = await makeTempDir();
    const githubEnv = join(dir, "github-env.txt");
    ...
    await exportGithubEnv(
      {
        ...
        VITE_SITE_URL: "https://mytrainerconnect.com",
      },
      {
        githubEnv,
        outputKeys: [],
    ...
    expect(await readFile(githubEnv, "utf8")).toBe("VITE_SITE_URL=https://mytrainerconnect.com\n");
  });
