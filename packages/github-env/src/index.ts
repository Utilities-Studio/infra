...
function createCli() {
  ...
  cli.option("--skip-reserved", "Skip GitHub-reserved env keys instead of failing");
...
function parseArgs(version: string): Args | null {
  ...
  return {
    ...
    skipReserved: Boolean(options.skipReserved),
  };
}
...
function isWritableEnvKey(key: string): boolean {
  if (
...
export async function exportGithubEnv(
  env: Record<string, string>,
  options: ExportGithubEnvOptions,
): Promise<void> {
  const writeCommand = options.writeCommand ?? ((command) => console.log(command));
  const sortedEntries = Object.entries(env).sort(([left], [right]) => left.localeCompare(right));
  const entries = options.skipReserved
    ? sortedEntries.filter(([key]) => isWritableEnvKey(key))
    : sortedEntries;
...
async function main() {
  ...
  await exportGithubEnv(env, {
    ...
    skipReserved: args.skipReserved,
  });
  ...
  if (!args.quiet) {
    ...
    info(
      `github-env: loaded ${Object.keys(env).length} keys from ${envLabel}; wrote ${outputCount} outputs.`,
    );
  }
}
